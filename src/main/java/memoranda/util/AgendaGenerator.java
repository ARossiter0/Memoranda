...
public class AgendaGenerator {
	...
	static String generateTasksInfo(Project p, CalendarDate date, Collection expandedTasks) {
		...
		s += generateIndividualTasksInfo("Assignments", p, date, getAssignmentList(p), assigns);
	...
	static String generateLecturesInfo(Project p, CalendarDate date, Collection lectures){
		String s = "";
		...
		if (lectures.size() == 0) {
			...
			s += Local.getString(descr + " (" + lectures.size() + ")") + ":<br>\n<ul>\n";
	...
	static String generateIndividualTasksInfo(String descr, Project p, CalendarDate date, TaskList tl, Collection expandedTasks){
		String s = "";
