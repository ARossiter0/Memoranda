...
public class LectureImpl implements Task, Comparable {
     ...
     */
    public LectureImpl(Element taskElement, TaskList tl) {
	...
	public Task getParentTask() {
    	...
    	if (parentNode instanceof Element) {
        	...
        	if (parent.getLocalName().equalsIgnoreCase("task")) 
        	    return new LectureImpl(parent, _tl);
    	}
    	return null;
	}
...
*/
	...
	private Collection convertToTaskObjects(Elements tasks) {
        ...
        for (int i = 0; i < tasks.size(); i++) {
            Task t = new LectureImpl(tasks.get(i), _tl);
            v.add(t);
        }
        return v;
    }
	...
	public Task getSubTask(String id) {
		...
		for (int i = 0; i < subTasks.size(); i++) {
			if (subTasks.get(i).getAttribute("id").getValue().equals(id))
				return new LectureImpl(subTasks.get(i), _tl);
		}
		return null;
	}
